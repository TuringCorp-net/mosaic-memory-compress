...
## 14. 致命发现：持续压缩与 DeepSeek 前缀缓存的不可调和（2026-08-16）

**实测**：压缩当次请求缓存命中率 99.7% → **4.2%**（29 万 token 整段 miss，
30 倍价）；恢复后 99.9%。会话成本约 10 倍（用户体感：1-2 元 → ~10 元/上午）。

**根因**：DeepSeek 自动前缀缓存按"从头逐条一致"匹配——任何对已发送历史
的中间修改（替换/折叠）都使 breakpoint 前移，被修改点之后整段失效。
压缩省下的 token 远抵不过 miss 税。

**数学**：压缩税 = surface×30/N。N=10 时每轮摊 90 万单位 vs 无压缩增量
税 6 万——15 倍；N=100 才打平，但等于前 100 轮无感（算法价值归零）。

**决策**：持续压缩形态暂停下线。v2 方向 = 重置时刻增强（见 ROADMAP M5）：
只在新会话/brief 时刻注入精炼近期轮——缓存成本为零（搭新会话全 miss
的便车），哲学保留（近期记忆鲜活）。

**教训（通用）**：任何上下文压缩算法都必须先回答"缓存断点代价"——
在自动前缀缓存 provider 上，中间修改历史的方案在经济上不可行。
