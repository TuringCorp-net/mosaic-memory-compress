...
function validateConfig(config) {
    if (!Number.isInteger(config.lightStart) || config.lightStart < 0) {
        throw new TypeError(`[mosaic-memory-compress] lightStart must be a non-negative integer, got ${config.lightStart}`);
    }
    if (!Number.isInteger(config.lightWindow) || config.lightWindow <= 0) {
        throw new TypeError(`[mosaic-memory-compress] lightWindow must be a positive integer, got ${config.lightWindow}`);
    }
    if (!Number.isInteger(config.heavyWindow) || config.heavyWindow <= 0) {
        throw new TypeError(`[mosaic-memory-compress] heavyWindow must be a positive integer, got ${config.heavyWindow}`);
    }
    if (!Number.isInteger(config.heavyStart) || config.heavyStart <= config.lightStart) {
        throw new TypeError(`[mosaic-memory-compress] heavyStart (${config.heavyStart}) must be an integer greater than lightStart (${config.lightStart})`);
    }
}
// ============================================================
// Main entry point
 ...
 */
export async function mosaicMemoryCompress(messages, config) {
...
// Round counting
// ============================================================
function findRoundStarts(history) {
    const starts = [];
    for (let i = 0; i < history.length; i++) {
        ...
        if (history[i].role === 'user' && !history[i]._heavy)
            starts.push(i);
    }
    return starts;
}
...
// ============================================================
async function applyLightCompress(history, roundStarts, heavyEnd, lightEnd, round, config) {
...
async function runLightCompressLLM(messages, _config) {
    return messages.map((m) => {
        ...
        if (m.role === 'tool') {
            ...
            if (text.length > LIGHT_RESULT_HEAD + LIGHT_RESULT_TAIL)
                next.content = truncateToolResult(text);
        }
...
// ============================================================
async function applyHeavyCompress(history, roundStarts, heavyEnd, round, config) {
...
async function emitCompressEvent(config, event) {
    ...
    try {
        ...
        console.error('[mosaic-memory-compress] onCompress callback failed:', err.message);
    }
}
async function runHeavyCompressLLM(messages, config) {
...
[{"role": "user", "content": "<summary>"}, {"role": "assistant", "content": "<confirmation>"}]`;
    try {
        ...
        console.error('[mosaic-memory-compress] Heavy Compress LLM call failed:', err.message);
        return [
...
function parseHeavyResult(raw) {
    try {
        ...
        if (pair.length !== 2)
            throw new Error('Expected 2 normalized messages');
        return pair;
    }
    catch {
        return [
